# Kurdish Font Library

Kurdish typefaces, packaged for desktop, print and web.

**[fonts.daban.one](https://fonts.daban.one)** — preview them with your own text
and download.

## Fonts

| Family | Designer | Weights | Version |
| --- | --- | --- | --- |
| [Lenos](fonts/Lenos) | Sirwan Yassin | Thin → Black (9) | 1.002 |
| [Aveno](fonts/Aveno) | Rabar | Regular, Bold | 1.001 |
| [K24](fonts/K24) | RTLtype | Light, Bold | 1.001 |

All three cover the full Central Kurdish (Sorani) alphabet — ڕ ڵ ۆ ێ ڤ ھ ە —
with initial, medial and final forms for every joining letter. Lenos and K24 also
cover the Kurmanji (Hawar) Latin alphabet; Aveno is Sorani only.

## Formats

Every family ships in four formats:

| | |
| --- | --- |
| `OTF/` | print and publishing |
| `TTF/` | desktop installation |
| `WEB/WOFF2/` | web |
| `WEB/WOFF/` | web, older browsers |

Download a family from [fonts.daban.one](https://fonts.daban.one), or take
individual files from `fonts/`.

## Licensing

**These typefaces are other people's work.** This repository claims no rights
over them and grants none. No permission to redistribute was obtained, and K24
carries an explicit "All rights reserved" notice from its author.

All rights remain with the original designers. Obtain permission from the
designer before using these fonts.

If you are one of the designers and want your work removed, please open an issue
and it will be removed.

See [LICENSING.md](LICENSING.md) for the position on each family.

## Credits

Typefaces by **Sirwan Yassin** (Lenos), **Rabar** (Aveno) and **RTLtype** (K24).

Packaging and metadata corrections by Daban, 2026.
